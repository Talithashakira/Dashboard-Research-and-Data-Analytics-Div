...
# --- State: persist tombol submit agar tidak hilang saat rerun di Cloud ---
...
if uploaded_file is not None:
    ...
    if not has_visit:
        ...
        with st.form("visit_date_filter", clear_on_submit=False):
            ...
            if st.form_submit_button("Filter & Ekstrak Pelanggan"):
                st.session_state.extract_clicked = True

        # Jika belum klik submit, tampilkan full range agar hasil tetap terlihat
        if not st.session_state.extract_clicked:
            df_filtered = df.copy()
            st.info("Menampilkan hasil untuk seluruh rentang tanggal. Klik **Filter & Ekstrak Pelanggan** untuk menyaring.")
        else:
    ...
    # --- Ekstraksi pelanggan unik per unit ---
    customers_dict = extract_unique_customers(df_filtered)
    ...
    if not customers_dict:
        ...
        # --- Debug opsional di Cloud ---
        with st.expander("🔍 Debug info (opsional)"):
            st.write("Units unik pada data:", sorted(df_filtered["Ticket Group"].dropna().unique().tolist()))
            st.write("Jumlah baris setelah filter:", len(df_filtered))
